...
Rules:
- STRICTLY A2 level — NOT beginner (A1). Avoid ultra-basic words like cat, dog, bed, run, tea.
- Choose words that A2 students are learning: emotions, travel, shopping, health, work, hobbies, weather, food variety, etc.
- Examples of good A2 words: nervous, receipt, luggage, appointment, schedule, recommend, describe, improve
- Common everyday words that are useful in real conversations
- Ukrainian must use only Ukrainian letters, no Russian
...
async def scheduler(redis_client):
    ...
    published_today = set()  # зберігаємо "дата_година" щоб не дублювати після рестарту

    while True:
        now  = datetime.now()
        hour = now.hour
        today_key = f"{now.strftime('%Y-%m-%d')}_{hour}"

        # Перевіряємо чи є рубрика для цієї години і чи ще не публікували сьогодні
        if hour in SCHEDULE and today_key not in published_today:
            # Публікуємо тільки якщо зараз перша хвилина години або бот щойно запустився
            if now.minute == 0 or today_key not in published_today:
                rubric = SCHEDULE[hour]
                published_today.add(today_key)
