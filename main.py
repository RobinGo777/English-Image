...
""",
        ...
        "quote": f"""
You are an English teacher. Find a wise quote for A2 level English students.
{history_note}
Return ONLY valid JSON, no markdown, no extra text:
{{
  "quote_en": "the COMPLETE quote in English — must be at least 6 words, never just 2-3 words",
  "author": "author name",
  "quote_ua": "full Ukrainian translation — must match the complete English quote"
}}
Rules:
- The quote MUST be complete and meaningful — NEVER truncated (not just "Dream big" or "Be kind")
- Good examples: "The only way to do great work is to love what you do", "In the middle of difficulty lies opportunity"
- Bad examples: "Dream big", "Be the change", "Work hard" — these are TOO SHORT, REJECTED
- Simple A2 vocabulary
- Well-known author
- Ukrainian must use only Ukrainian letters, no Russian
""",
...
""",
        ...
        "motivation": f"""
You are an English teacher. Find a motivational quote in simple English for A2 level students.
{history_note}
Return ONLY valid JSON, no markdown, no extra text:
{{
  "quote_en": "the COMPLETE motivational quote — must be at least 6 words, never just 2-3 words",
  "author": "author name",
  "quote_ua": "full Ukrainian translation — must match the complete English quote"
}}
Rules:
- The quote MUST be complete and meaningful — NEVER truncated (not just "Dream big" or "Never give up")
- Good examples: "It does not matter how slowly you go as long as you do not stop", "Success is not final, failure is not fatal"
- Bad examples: "Dream big", "Keep going", "Work hard" — these are TOO SHORT, REJECTED
...
async def self_ping():
    """Бот сам себе пінгує кожні 10 хвилин щоб Render не засинав."""
    port = int(os.environ.get("PORT", 10000))
...
if __name__ == "__main__":
    ...
    if result.returncode == 0:
        ...
        log.error(f"❌ Chromium install failed: {result.stderr}")
    asyncio.run(main())
